import { createSignal, createMemo, createRenderEffect, on, useContext, runWithOwner, createContext, getOwner, startTransition, resetErrorBoundaries, batch, untrack, createComponent } from 'file:///home/engr_luis/Projects/kahitsan-web/node_modules/solid-js/dist/server.js';
import { isServer, getRequestEvent } from 'file:///home/engr_luis/Projects/kahitsan-web/node_modules/solid-js/web/dist/server.js';
...
const Ae = 100, Ce = createContext(), ee = createContext(), L = () => Re(useContext(Ce), "<A> and 'use' router primitives can be only used inside a Route."), Ee = () => useContext(ee) || L().base, We = (t) => {
  const e = Ee();
...
}, Ie = () => L().navigatorFactory(), Me = () => L().location, De = () => L().params;
function Le(t, e = "") {
...
function ze(t, e, n, s = {}) {
  const { signal: [r, o], utils: a = {} } = t, c = a.parsePath || ((i) => i), f = a.renderPath || ((i) => i), h = a.beforeLeave || ge(), l = W("", s.base || "");
  ...
  }, [v, C] = createSignal(r().value), [S, ne] = createSignal(r().state), O = Fe(v, S, a.queryWrapper), F = [], U = createSignal(isServer ? ce() : []), z = createMemo(() => typeof s.transformUrl == "function" ? I(e(), s.transformUrl(O.pathname)) : I(e(), O.pathname)), H = () => {
    const i = z(), u = {};
  ...
  return createRenderEffect(on(r, (i) => y("native", i), { defer: true })), { base: K, location: O, params: re, isRouting: m, renderPath: f, parsePath: c, navigatorFactory: oe, matches: z, beforeLeave: h, preloadRoute: ie, singleFlight: s.singleFlight === void 0 ? true : s.singleFlight, submissions: U };
  function se(i, u, g) {
    untrack(() => {
      if (typeof u == "number") {
      ...
      const P = !u || u[0] === "?", { replace: _, resolve: x, scroll: j, state: b } = { replace: false, resolve: !P, scroll: true, ...g }, A = x ? i.resolvePath(u) : W(P && O.pathname || "", u);
      if (A === void 0) throw new Error(`Path '${u}' is not a routable path`);
      if (F.length >= Ae) throw new Error("Too many redirects");
      const N = v();
      if (A !== N || b !== S()) if (isServer) {
        const T = getRequestEvent();
        T && (T.response = { status: 302, headers: new Headers({ Location: A }) }), o({ value: A, replace: _, scroll: j, state: b });
      } else h.confirm(A, g) && (F.push({ value: N, replace: _, scroll: j, state: S() }), y("navigate", { value: A, state: b }));
    });
  }
  function oe(i) {
  ...
  function ae(i) {
    const u = F[0];
    u && (o({ ...i, replace: u.replace, scroll: u.scroll }), F.length = 0);
  }
  function ie(i, u) {
...
export { $e as $, Be as B, Ce as C, De as D, E, He as H, Ie as I, Me as M, Oe as O, Q, Ue as U, We as W, I as a, ee as e, ge as g, qe as q, ve as v, ze as z };
//# sourceMappingURL=routing-BSRz0T-p.mjs.map
