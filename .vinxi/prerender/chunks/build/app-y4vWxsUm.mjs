...
import { Q as Qe, J as Je, l as lt$1, k as ke, _ as _e, c as ct$1, u as ut$1, I, b as le, t as tt, d as se, M as Me, n as nt } from './routing-CzLrmEq0.mjs';
...
const T = (t) => (r) => {
  const { base: o } = r, n = children(() => r.children), e = createMemo(() => Je(n(), r.base || ""));
  let s;
  const u = lt$1(t, e, () => s, { base: o, singleFlight: r.singleFlight, transformUrl: r.transformUrl });
  return t.create && t.create(u), createComponent(ke.Provider, { value: u, get children() {
    return createComponent(ot, { routerState: u, get root() {
      return r.root;
    }, get preload() {
...
function ot(t) {
  const r = t.routerState.location, o = t.routerState.params, n = createMemo(() => t.preload && untrack(() => {
    t.preload({ params: o, location: r, intent: ct$1() || "initial" });
  }));
...
function at(t) {
  ...
  const n = createMemo(on(t.routerState.matches, (e, s, u) => {
    ...
    for (let l = 0, w = e.length; l < w; l++) {
      const p = s && s[l], g = e[l];
      u && p && g.route.key === p.route.key ? m[l] = u[l] : (f = false, r[l] && r[l](), createRoot((R) => {
        r[l] = R, m[l] = ut$1(t.routerState, m[l - 1] || t.routerState.base, P(() => n()[l + 1]), () => {
          var _a;
...
function ut(t) {
  let r = false;
  const o = (e) => typeof e == "string" ? { value: e } : e, n = st(createSignal(o(t.get()), { equals: (e, s) => e.value === s.value && e.state === s.state }), void 0, (e) => (!r && t.set(e), sharedConfig.registry && !sharedConfig.done && (sharedConfig.done = true), e));
...
function ht(t) {
  let r;
  const o = { value: t.url || (r = getRequestEvent()) && dt(r.request.url) || "" };
  return T({ signal: [() => o, (n) => Object.assign(o, n)] })(t);
}
const mt = /* @__PURE__ */ new Map();
function ft({ preload: t = true, explicitLinks: r = false, actionBase: o = "/_server", transformUrl: n } = {}) {
  return (e) => {
...
function gt(t) {
  ...
  return ut({ get: r, set({ value: n, replace: e, scroll: s, state: u }) {
    e ? window.history.replaceState(tt(u), "", n) : window.history.pushState(u, "", n), lt(decodeURIComponent(window.location.hash.slice(1)), s), se();
  }, init: (n) => ct(window, "popstate", nt(n, (e) => {
    if (e) return !o.confirm(e);
    {
      const s = r();
      return !o.confirm(s.value, { state: s.state });
    }
  })), create: ft({ preload: t.preload, explicitLinks: t.explicitLinks, actionBase: t.actionBase, transformUrl: t.transformUrl }), utils: { go: (n) => window.history.go(n), beforeLeave: o } })(t);
}
var wt = ["<div", ' class="page-transition-container relative min-h-screen">', "</div>"];
function pt(t) {
  return ssr(wt, ssrHydrationKey(), escape(createComponent(Transition, { name: "slide-up", mode: "outin", get children() {
...
function Ft() {
  return createComponent(Qe, { get children() {
    return createComponent(gt, { root: pt, get children() {
...
//# sourceMappingURL=app-y4vWxsUm.mjs.map
