import { isServer, createComponent, useAssets, ssr, spread, escape, getRequestEvent } from 'file:///home/engr_luis/Projects/kahitsan-web/node_modules/solid-js/web/dist/server.js';
import { sharedConfig, createContext, createSignal, createMemo, createRenderEffect, on, useContext, runWithOwner, createUniqueId, getOwner, startTransition, resetErrorBoundaries, batch, untrack, createComponent as createComponent$1, onCleanup } from 'file:///home/engr_luis/Projects/kahitsan-web/node_modules/solid-js/dist/server.js';

const re = createContext(), oe = ["title", "meta"], W = [], K = ["name", "http-equiv", "content", "charset", "media"].concat(["property"]), F = (n, t) => {
  const e = Object.fromEntries(Object.entries(n.props).filter(([r]) => t.includes(r)).sort());
  return (Object.hasOwn(e, "name") || Object.hasOwn(e, "property")) && (e.name = e.name || e.property, delete e.property), n.tag + JSON.stringify(e);
};
function Se() {
  if (!sharedConfig.context) {
    const e = document.head.querySelectorAll("[data-sm]");
    Array.prototype.forEach.call(e, (r) => r.parentNode.removeChild(r));
  }
  const n = /* @__PURE__ */ new Map();
  function t(e) {
    if (e.ref) return e.ref;
    let r = document.querySelector(`[data-sm="${e.id}"]`);
    return r ? (r.tagName.toLowerCase() !== e.tag && (r.parentNode && r.parentNode.removeChild(r), r = document.createElement(e.tag)), r.removeAttribute("data-sm")) : r = document.createElement(e.tag), r;
  }
  return { addTag(e) {
    if (oe.indexOf(e.tag) !== -1) {
      const s = e.tag === "title" ? W : K, a = F(e, s);
      n.has(a) || n.set(a, []);
      let i = n.get(a), d = i.length;
      i = [...i, e], n.set(a, i);
      let f = t(e);
      e.ref = f, spread(f, e.props);
      let l = null;
      for (var r = d - 1; r >= 0; r--) if (i[r] != null) {
        l = i[r];
        break;
      }
      return f.parentNode != document.head && document.head.appendChild(f), l && l.ref && l.ref.parentNode && document.head.removeChild(l.ref), d;
    }
    let o = t(e);
    return e.ref = o, spread(o, e.props), o.parentNode != document.head && document.head.appendChild(o), -1;
  }, removeTag(e, r) {
    const o = e.tag === "title" ? W : K, s = F(e, o);
    if (e.ref) {
      const a = n.get(s);
      if (a) {
        if (e.ref.parentNode) {
          e.ref.parentNode.removeChild(e.ref);
          for (let i = r - 1; i >= 0; i--) a[i] != null && document.head.appendChild(a[i].ref);
        }
        a[r] = null, n.set(s, a);
      } else e.ref.parentNode && e.ref.parentNode.removeChild(e.ref);
    }
  } };
}
function Te() {
  const n = [];
  return useAssets(() => ssr(Ne(n))), { addTag(t) {
    if (oe.indexOf(t.tag) !== -1) {
      const e = t.tag === "title" ? W : K, r = F(t, e), o = n.findIndex((s) => s.tag === t.tag && F(s, e) === r);
      o !== -1 && n.splice(o, 1);
    }
    return n.push(t), n.length;
  }, removeTag(t, e) {
  } };
}
const Qe = (n) => {
  const t = isServer ? Te() : Se();
  return createComponent(re.Provider, { value: t, get children() {
    return n.children;
  } });
}, H = (n, t, e) => (Le({ tag: n, props: t, setting: e, id: createUniqueId(), get name() {
  return t.name || t.property;
} }), null);
function Le(n) {
  const t = useContext(re);
  if (!t) throw new Error("<MetaProvider /> should be in the tree");
  createRenderEffect(() => {
    const e = t.addTag(n);
    onCleanup(() => t.removeTag(n, e));
  });
}
function Ne(n) {
  return n.map((t) => {
    var _a, _b;
    const r = Object.keys(t.props).map((s) => s === "children" ? "" : ` ${s}="${escape(t.props[s], true)}"`).join("");
    let o = t.props.children;
    return Array.isArray(o) && (o = o.join("")), ((_a = t.setting) == null ? void 0 : _a.close) ? `<${t.tag} data-sm="${t.id}"${r}>${((_b = t.setting) == null ? void 0 : _b.escape) ? escape(o) : o || ""}</${t.tag}>` : `<${t.tag} data-sm="${t.id}"${r}/>`;
  }).join("");
}
const Ye = (n) => H("title", n, { escape: true, close: true }), Ze = (n) => H("meta", n), et = (n) => H("link", n);
function Me() {
  let n = /* @__PURE__ */ new Set();
  function t(o) {
    return n.add(o), () => n.delete(o);
  }
  let e = false;
  function r(o, s) {
    if (e) return !(e = false);
    const a = { to: o, options: s, defaultPrevented: false, preventDefault: () => a.defaultPrevented = true };
    for (const i of n) i.listener({ ...a, from: i.location, retry: (d) => {
      d && (e = true), i.navigate(o, { ...s, resolve: false });
    } });
    return !a.defaultPrevented;
  }
  return { subscribe: t, confirm: r };
}
let U;
function se() {
  (!window.history.state || window.history.state._depth == null) && window.history.replaceState({ ...window.history.state, _depth: window.history.length - 1 }, ""), U = window.history.state._depth;
}
isServer || se();
function tt(n) {
  return { ...n, _depth: window.history.state && window.history.state._depth };
}
function nt(n, t) {
  let e = false;
  return () => {
    const r = U;
    se();
    const o = r == null ? null : U - r;
    if (e) {
      e = false;
      return;
    }
    o && t(o) ? (e = true, window.history.go(-o)) : n();
  };
}
const qe = /^(?:[a-z0-9]+:)?\/\//i, Fe = /^\/+|(\/)\/+$/g, _e = "http://sr";
function E(n, t = false) {
  const e = n.replace(Fe, "$1");
  return e ? t || /^[?#]/.test(e) ? e : "/" + e : "";
}
function q(n, t, e) {
  if (qe.test(t)) return;
  const r = E(n), o = e && E(e);
  let s = "";
  return !o || t.startsWith("/") ? s = r : o.toLowerCase().indexOf(r.toLowerCase()) !== 0 ? s = r + o : s = o, (s || "/") + E(t, !s);
}
function Be(n, t) {
  if (n == null) throw new Error(t);
  return n;
}
function Ie(n, t) {
  return E(n).replace(/\/*(\*.*)?$/g, "") + E(t);
}
function ae(n) {
  const t = {};
  return n.searchParams.forEach((e, r) => {
    r in t ? Array.isArray(t[r]) ? t[r].push(e) : t[r] = [t[r], e] : t[r] = e;
  }), t;
}
function We(n, t, e) {
  const [r, o] = n.split("/*", 2), s = r.split("/").filter(Boolean), a = s.length;
  return (i) => {
    const d = i.split("/").filter(Boolean), f = d.length - a;
    if (f < 0 || f > 0 && o === void 0 && !t) return null;
    const l = { path: a ? "" : "/", params: {} }, m = (p) => e === void 0 ? void 0 : e[p];
    for (let p = 0; p < a; p++) {
      const h = s[p], y = h[0] === ":", w = y ? d[p] : d[p].toLowerCase(), O = y ? h.slice(1) : h.toLowerCase();
      if (y && B(w, m(O))) l.params[O] = w;
      else if (y || !B(w, O)) return null;
      l.path += `/${w}`;
    }
    if (o) {
      const p = f ? d.slice(-f).join("/") : "";
      if (B(p, m(o))) l.params[o] = p;
      else return null;
    }
    return l;
  };
}
function B(n, t) {
  const e = (r) => r === n;
  return t === void 0 ? true : typeof t == "string" ? e(t) : typeof t == "function" ? t(n) : Array.isArray(t) ? t.some(e) : t instanceof RegExp ? t.test(n) : false;
}
function Ke(n) {
  const [t, e] = n.pattern.split("/*", 2), r = t.split("/").filter(Boolean);
  return r.reduce((o, s) => o + (s.startsWith(":") ? 2 : 3), r.length - (e === void 0 ? 0 : 1));
}
function ie(n) {
  const t = /* @__PURE__ */ new Map(), e = getOwner();
  return new Proxy({}, { get(r, o) {
    return t.has(o) || runWithOwner(e, () => t.set(o, createMemo(() => n()[o]))), t.get(o)();
  }, getOwnPropertyDescriptor() {
    return { enumerable: true, configurable: true };
  }, ownKeys() {
    return Reflect.ownKeys(n());
  }, has(r, o) {
    return o in n();
  } });
}
function ce(n) {
  let t = /(\/?\:[^\/]+)\?/.exec(n);
  if (!t) return [n];
  let e = n.slice(0, t.index), r = n.slice(t.index + t[0].length);
  const o = [e, e += t[1]];
  for (; t = /^(\/\:[^\/]+)\?/.exec(r); ) o.push(e += t[1]), r = r.slice(t[0].length);
  return ce(r).reduce((s, a) => [...s, ...o.map((i) => i + a)], []);
}
const Ue = 100, ke = createContext(), le = createContext(), $ = () => Be(useContext(ke), "<A> and 'use' router primitives can be only used inside a Route."), He = () => useContext(le) || $().base, rt = (n) => {
  const t = He();
  return createMemo(() => t.resolvePath(n()));
}, ot = (n) => {
  const t = $();
  return createMemo(() => {
    const e = n();
    return e !== void 0 ? t.renderPath(e) : e;
  });
}, st = () => $().navigatorFactory(), at = () => $().location, it = () => $().params;
function ze(n, t = "") {
  const { component: e, preload: r, load: o, children: s, info: a } = n, i = !s || Array.isArray(s) && !s.length, d = { key: n, component: e, preload: r || o, info: a };
  return ue(n.path).reduce((f, l) => {
    for (const m of ce(l)) {
      const p = Ie(t, m);
      let h = i ? p : p.split("/*", 1)[0];
      h = h.split("/").map((y) => y.startsWith(":") || y.startsWith("*") ? y : encodeURIComponent(y)).join("/"), f.push({ ...d, originalPath: l, pattern: h, matcher: We(h, !i, n.matchFilters) });
    }
    return f;
  }, []);
}
function De(n, t = 0) {
  return { routes: n, score: Ke(n[n.length - 1]) * 1e4 - t, matcher(e) {
    const r = [];
    for (let o = n.length - 1; o >= 0; o--) {
      const s = n[o], a = s.matcher(e);
      if (!a) return null;
      r.unshift({ ...a, route: s });
    }
    return r;
  } };
}
function ue(n) {
  return Array.isArray(n) ? n : [n];
}
function Je(n, t = "", e = [], r = []) {
  const o = ue(n);
  for (let s = 0, a = o.length; s < a; s++) {
    const i = o[s];
    if (i && typeof i == "object") {
      i.hasOwnProperty("path") || (i.path = "");
      const d = ze(i, t);
      for (const f of d) {
        e.push(f);
        const l = Array.isArray(i.children) && i.children.length === 0;
        if (i.children && !l) Je(i.children, f.pattern, e, r);
        else {
          const m = De([...e], r.length);
          r.push(m);
        }
        e.pop();
      }
    }
  }
  return e.length ? r : r.sort((s, a) => a.score - s.score);
}
function I(n, t) {
  for (let e = 0, r = n.length; e < r; e++) {
    const o = n[e].matcher(t);
    if (o) return o;
  }
  return [];
}
function Ve(n, t, e) {
  const r = new URL(_e), o = createMemo((l) => {
    const m = n();
    try {
      return new URL(m, r);
    } catch {
      return console.error(`Invalid path ${m}`), l;
    }
  }, r, { equals: (l, m) => l.href === m.href }), s = createMemo(() => o().pathname), a = createMemo(() => o().search, true), i = createMemo(() => o().hash), d = () => "", f = on(a, () => ae(o()));
  return { get pathname() {
    return s();
  }, get search() {
    return a();
  }, get hash() {
    return i();
  }, get state() {
    return t();
  }, get key() {
    return d();
  }, query: e ? e(f) : ie(f) };
}
let P;
function ct() {
  return P;
}
function lt(n, t, e, r = {}) {
  const { signal: [o, s], utils: a = {} } = n, i = a.parsePath || ((c) => c), d = a.renderPath || ((c) => c), f = a.beforeLeave || Me(), l = q("", r.base || "");
  if (l === void 0) throw new Error(`${l} is not a valid base path`);
  l && !o().value && s({ value: l, replace: true, scroll: false });
  const [m, p] = createSignal(false);
  let h;
  const y = (c, u) => {
    u.value === w() && u.state === j() || (h === void 0 && p(true), P = c, h = u, startTransition(() => {
      h === u && (O(h.value), fe(h.state), resetErrorBoundaries(), isServer || z[1]((g) => g.filter((x) => x.pending)));
    }).finally(() => {
      h === u && batch(() => {
        P = void 0, c === "navigate" && me(h), p(false), h = void 0;
      });
    }));
  }, [w, O] = createSignal(o().value), [j, fe] = createSignal(o().state), S = Ve(w, j, a.queryWrapper), T = [], z = createSignal(isServer ? ye() : []), D = createMemo(() => typeof r.transformUrl == "function" ? I(t(), r.transformUrl(S.pathname)) : I(t(), S.pathname)), J = () => {
    const c = D(), u = {};
    for (let g = 0; g < c.length; g++) Object.assign(u, c[g].params);
    return u;
  }, de = a.paramsWrapper ? a.paramsWrapper(J, t) : ie(J), V = { pattern: l, path: () => l, outlet: () => null, resolvePath(c) {
    return q(l, c);
  } };
  return createRenderEffect(on(o, (c) => y("native", c), { defer: true })), { base: V, location: S, params: de, isRouting: m, renderPath: d, parsePath: i, navigatorFactory: pe, matches: D, beforeLeave: f, preloadRoute: ge, singleFlight: r.singleFlight === void 0 ? true : r.singleFlight, submissions: z };
  function he(c, u, g) {
    untrack(() => {
      if (typeof u == "number") {
        u && (a.go ? a.go(u) : console.warn("Router integration does not support relative routing"));
        return;
      }
      const x = !u || u[0] === "?", { replace: L, resolve: R, scroll: N, state: b } = { replace: false, resolve: !x, scroll: true, ...g }, C = R ? c.resolvePath(u) : q(x && S.pathname || "", u);
      if (C === void 0) throw new Error(`Path '${u}' is not a routable path`);
      if (T.length >= Ue) throw new Error("Too many redirects");
      const X = w();
      if (C !== X || b !== j()) if (isServer) {
        const G = getRequestEvent();
        G && (G.response = { status: 302, headers: new Headers({ Location: C }) }), s({ value: C, replace: L, scroll: N, state: b });
      } else f.confirm(C, g) && (T.push({ value: X, replace: L, scroll: N, state: j() }), y("navigate", { value: C, state: b }));
    });
  }
  function pe(c) {
    return c = c || useContext(le) || V, (u, g) => he(c, u, g);
  }
  function me(c) {
    const u = T[0];
    u && (s({ ...c, replace: u.replace, scroll: u.scroll }), T.length = 0);
  }
  function ge(c, u) {
    const g = I(t(), c.pathname), x = P;
    P = "preload";
    for (let L in g) {
      const { route: R, params: N } = g[L];
      R.component && R.component.preload && R.component.preload();
      const { preload: b } = R;
      u && b && runWithOwner(e(), () => b({ params: N, location: { pathname: c.pathname, search: c.search, hash: c.hash, query: ae(c), state: null, key: "" }, intent: "preload" }));
    }
    P = x;
  }
  function ye() {
    const c = getRequestEvent();
    return c && c.router && c.router.submission ? [c.router.submission] : [];
  }
}
function ut(n, t, e, r) {
  const { base: o, location: s, params: a } = n, { pattern: i, component: d, preload: f } = r().route, l = createMemo(() => r().path);
  d && d.preload && d.preload();
  const m = f ? f({ params: a, location: s, intent: P || "initial" }) : void 0;
  return { parent: t, pattern: i, path: l, outlet: () => d ? createComponent$1(d, { params: a, location: s, data: m, get children() {
    return e();
  } }) : e(), resolvePath(h) {
    return q(o.path(), h, l());
  } };
}

export { E, I, Je as J, Me as M, Qe as Q, Ye as Y, Ze as Z, _e as _, at as a, le as b, ct as c, se as d, et as e, it as i, ke as k, lt as l, nt as n, ot as o, rt as r, st as s, tt as t, ut as u };
//# sourceMappingURL=routing-CzLrmEq0.mjs.map
